...
ran first in the same session. Root cause: `platform/semantic_api/dependencies.py` opened its
...
(`platform/semantic_api/dependencies.py`), fully isolated from the test suite's own `"default"`
...
platform/ingestion/.venv/bin/python platform/ingestion/ingest.py --limit 500

platform/enrichment/.venv/bin/python platform/enrichment/enrich.py

platform/semantic_api/.venv/bin/python -m uvicorn semantic_api.main:app --app-dir platform --port 8010 &

usecases/8_1_batch_reactive/recommender/.venv/bin/python usecases/8_1_batch_reactive/recommender/recommend.py

platform/enrichment/.venv/bin/python -m pytest   # full suite against the rebuilt stack (tests/ + usecases/)
```
...
-m pytest`). Dataset: 411 real, licensed tracks (within the 200-500 seed
