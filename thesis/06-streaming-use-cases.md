...
![Figure 6.10: 8.2's demonstration page, live against the running example. Left: the session's three played tracks (blue, connected in play order) against the same real PCA projection of the catalog Figure 5.7 uses, with the session's centroid position (green) and its resulting recommendations (gold) connected to it. Right: session state, showing a computed 512-dimensional profile from three events (the Flink job of Section 6.1.4 has already fired a window), and the ranked list itself, computed by the warm path of Figure 6.3 over the session profile rather than over any single track. The centroid marker's position is the mean of the three played tracks' own 2D positions, a visual approximation for this scatter -- not a projection of the real 512-dimensional weighted centroid, which lives in a space this 2D view cannot show exactly.](figures/fig-6-10-uc82-demo.png){width=6.3in}
...
![Figure 6.11: Use case 8.3's minimal demonstration slice, live against the running example. Left: "Give Me Hope," the currently-playing track, highlighted on the same catalog projection Figures 5.7 and 6.10 use, connected live to the pushed suggestion the moment it is computed. Right: the live classifier's top-3 CLAP zero-shot tags (artrock/country/poprock) are a real, imperfect result on this small amateur catalog, not a cherry-picked one; the pushed suggestion is the same-artist, genre-sibling, high-similarity track the batch signals of Chapter 5 already favour.](figures/fig-6-11-uc83-demo.png){width=6.3in}
