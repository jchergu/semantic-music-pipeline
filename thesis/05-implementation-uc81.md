...
Sections 5.1–5.6 describe 8.1 from the outside (the batch run's results) and the inside (its module structure). This section shows it running, against the running example introduced in Section 1.3: the seed track is "Gates" (#14, Bellevue), queried live.

![Figure 5.7: 8.1's demonstration page, live against the running example. Left: the seed track "Gates" (blue, ringed) plotted against a real PCA projection of all 411 catalog embeddings, connected to its top-10 recommendations (gold) as they're computed. Right: the same ranked list, with a per-row breakdown of which of the three signals — CLAP similarity, genre-sibling status, same-artist status — contributed to each score, the same three signals Section 5.6.1's ranking module always combines, called here through a thin demo-only wrapper (`usecases/8_1_batch_reactive/demo_api.py`) rather than run as a batch, but computing nothing this chapter's own recommender does not already compute.](figures/fig-5-7-uc81-demo.png){width=6.3in}

The top result, "Give Me Hope" (#16), is both acoustically similar to "Gates" (CLAP cosine 0.843) and a genre sibling — the two signals stacking, exactly as Section 5.6.1's additive scoring is designed to do. This is not a separate code path built for the screenshot: the demo endpoint calls the same `context_builder.build_context()` and `ranking.score_recommendations()` functions Section 5.6.1 already describes, over the same frozen Semantic API contract of Section 5.6.3. Chapter 6 continues this exact thread — "Gates," "Billy Comes Home," and "Give Me Hope" playing in sequence — to show what changes once the same three signals are computed over a *session* instead of a single seed.

The scatter plot behind the ranked list is a real PCA projection of all 411 tracks' 512-dimensional CLAP embeddings down to two dimensions (`demo/precompute_embedding_space.py`), not a decorative layout: two components explain only 22% of the embedding space's variance, so proximity on the plot is a rough visual cue, not a claim that two nearby points are exactly as similar as two points that far apart in the real 512-dimensional space Milvus actually searches. Figures 6.10 and 6.11 reuse the identical projection, so a track's position is consistent across all three demonstration pages.

The secondary pair from Section 1.3 makes the same-artist signal concrete in a way this rock thread cannot, since none of its three tracks share an artist. Querying "Wish You Were Here" (#1, The.madpix.project) returns "Moments" (#12, the same artist) in first place, with a raw CLAP similarity of only 0.458 — lower than the second-ranked candidate's 0.584. The genre-sibling and same-artist boosts (0.05 and 0.15) together outweigh that gap, exactly the re-ranking behaviour Section 5.6.1 describes boosts existing for: pulling a track the acoustic signal alone would have placed lower into the top spot, on the strength of a high-precision structural signal a purely content-based comparison cannot see.
