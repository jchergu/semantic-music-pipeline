...
![Figure 5.7: 8.1's demonstration page, live against the running example. The seed track "Gates" returns a ranked list combining CLAP similarity (blue badges), genre-sibling status (orange), and same-artist status (green) — the same three signals Section 5.6.1's ranking module always combines, called here through a thin demo-only wrapper (`usecases/8_1_batch_reactive/demo_api.py`) rather than run as a batch, but computing nothing this chapter's own recommender does not already compute.](figures/fig-5-7-uc81-demo.png){width=6.3in}
