...
Also this session: every chapter now starts on a new page (commit `90e9002`,
`merge_frontmatter.py`, see the Working agreement's `build/` bullet). Verified
by rendering the built `.docx` to PDF: 64 pages, abstract on 3, chapters 1-7
...
- `build/` is gitignored build output — **never edit `build/thesis.docx`
  ...
  `thesis/merge_frontmatter.py` splices it in front of pandoc's output,
  relocates pandoc's TOC to sit after it (pandoc always emits the TOC first
  and offers no way to place it), and gives every Heading 1 a page break so
  each chapter opens a page. That last rule lives there rather than on the
  Heading 1 style in `reference.docx` so it stays visible in a text diff and
  survives the style document being regenerated by hand in LibreOffice. The Makefile refuses to build while
